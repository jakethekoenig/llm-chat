...
test('renders code block content during streaming', async () => {
  ...
  expect(await screen.findByText((content, element) => element !== null && element.tagName.toLowerCase() === 'code' && content.includes("console"))).toBeInTheDocument();
  expect(await screen.findByText((content, element) => element !== null && element.tagName.toLowerCase() === 'code' && content.includes("log"))).toBeInTheDocument();
