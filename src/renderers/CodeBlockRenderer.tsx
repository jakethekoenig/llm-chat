...
export class CodeBlockRenderer implements Renderer {
  detectStartSequence(content: string, startIndex: number): number | [number, number] | null {
    const start = content.indexOf('```', startIndex);
    return start === -1 ? null : [start, start + 3];
  }

  detectEndSequence(content: string, startIndex: number): number | [number, number] | null {
